...
struct FGaussianComponentChunk
{
	...
	UTexture* rot;
	UTexture* scale;
	// Use float (32-bit) for higher precision and clearer splat rendering (was FFloat16)
	TArray<float> col_data;
	TArray<float> pos_data;
	TArray<float> rot_data;
	TArray<float> scale_data;
	uint32 num;
	FVector3f center;
	FVector3f extent;
