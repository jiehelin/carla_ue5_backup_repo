...
void Setup2DTexture(UTexture2D*& OutTexture, EPixelFormat PixelFormat, FIntPoint Dimensions, uint8* InSourceArray, TextureFilter Filter)
{
	...
	// Use 32F source when pixel format is 32-bit float for higher precision Gaussian splats
	const ETextureSourceFormat SourceFormat = (PixelFormat == PF_A32B32G32R32F) ? TSF_RGBA32F : TSF_RGBA16F;
	OutTexture->Source.Init(Dimensions.X, Dimensions.Y, 1, 1, SourceFormat, InSourceArray);
