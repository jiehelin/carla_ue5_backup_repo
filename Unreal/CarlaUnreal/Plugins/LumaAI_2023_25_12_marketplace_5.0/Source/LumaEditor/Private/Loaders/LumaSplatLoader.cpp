...
// 32-bit float version for higher precision and clearer splat rendering (replaces F16)
template<typename F>
TArray<float> InitializeF32GaussianTextureDataPerTexel(uint32 texture_width, uint32 texture_height, F&& per_texel_lambda)
{
	TArray<float> texture_data;
	texture_data.Reserve(texture_width * texture_height * 4);

	uint32 num_texels = texture_width * texture_height;
	for (uint32 flat_index = 0; flat_index < num_texels; flat_index++)
	{
		int success = per_texel_lambda(flat_index, texture_data);
		if (success == 2)
		{
			break;
		}
		if (success != 1)
		{
			texture_data.Add(0.0f);
			texture_data.Add(0.0f);
			texture_data.Add(0.0f);
			texture_data.Add(0.0f);
		}
	}
	// Pad remaining texels with zeros
	while (texture_data.Num() < num_texels * 4)
	{
		texture_data.Add(0.0f);
		texture_data.Add(0.0f);
		texture_data.Add(0.0f);
...
void CreateGaussianChunkLuma(const LumaRenderSDK::Gaussians& gaussians, const LumaRenderSDK::GaussianChunk& chunk, FGaussianComponentChunk& ret)
{
	...
	ret.pos_data = InitializeF32GaussianTextureDataPerTexel(texture_width, texture_height,
		[&] (uint32 flat_index, TArray<float>& texture_data)
		{
	...
	ret.col_data = InitializeF32GaussianTextureDataPerTexel(texture_width, texture_height,
		[&](uint32 flat_index, TArray<float>& texture_data)
		{
			if (flat_index >= num_gaussians)
				return 2;
	...
	ret.rot_data = InitializeF32GaussianTextureDataPerTexel(texture_width, texture_height,
		[&] (uint32 flat_index, TArray<float>& texture_data)
		{
			if (flat_index >= num_gaussians)
				return 2;
	...
	ret.scale_data = InitializeF32GaussianTextureDataPerTexel(texture_width, texture_height,
		[&] (uint32 flat_index, TArray<float>& texture_data)
		{
			if (flat_index >= num_gaussians)
				return 2;
			...
			if (chunk.num_gaussians != 0 && chunk.indices != nullptr)
				gaussian_index = chunk.indices[flat_index];
			auto& gaussian = gaussians.data[gaussian_index];
			FVector4f Scale(gaussian.scale.x / 32.0f, gaussian.scale.y / 32.0f, gaussian.scale.z / 32.0f, 1.0f);
			texture_data.Add(Scale.X);
			texture_data.Add(Scale.Y);
			texture_data.Add(Scale.Z);
			texture_data.Add(Scale.W);
			return 1;
		});
}
...
UObject* ULumaSplatLoader::CreateFromFileInExistingPackage(FString FileName, UObject* ParentPackage)
{
	...
	// Use 32-bit float texture format for higher precision and clearer splat rendering
	auto create_texture_asset = [&] (FString TextureName, FString TexturePackage, uint32 texture_width, uint32 texture_height, TArray<float>& texture_data, UTexture2D** tex)
	{
		constexpr EPixelFormat texture_format = PF_A32B32G32R32F;
